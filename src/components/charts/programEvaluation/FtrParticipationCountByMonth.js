...
const FtrParticipationCountByMonth = (props) => {
  const [chartLabels, setChartLabels] = useState([]);
  const [chartDataPoints, setChartDataPoints] = useState([]);

  const chartId = 'ftrParticipationCountByMonth';

  const processResponse = () => {
    const { ftrParticipationCountByMonth: countsByMonth } = props;
  ...
  useEffect(() => {
    processResponse();
  }, [props.ftrParticipationCountByMonth]);
      ...
      options={{
        ...
        scales: {
          ...
          yAxes: [{
            scaleLabel: {
              display: true,
              labelString: 'Participation count',
            },
          }],
        },
        tooltips: {
  ...
  if (header && (mostRecentValue !== null)) {
    const title = `There are <b style='color:#809AE5'>${mostRecentValue} active participants</b> in FTR this month.`;
...
export default FtrParticipationCountByMonth;
